...
public sealed class MemberAccessorGenerator : IIncrementalGenerator
{
        ...
        List<FactoryEntry> factories)
    {
        foreach (var classDeclarationSyntax in classes)
        {
            ...
            var factoryName = MakeFactoryName(classDeclarationSyntax);
            var targetName = MakeTargetName(ns, classDeclarationSyntax);
            var registryFactoryName = MakeRegistryFactoryName(ns, classDeclarationSyntax);
            factories.Add(new FactoryEntry(targetName, registryFactoryName));
    ...
    private static string MakeTargetName(string ns, ClassDeclarationSyntax syntax)
    {
        ...
        var suffix = syntax.TypeParameterList is null ? string.Empty : "<>";
        return $"{prefix}{syntax.Identifier}{suffix}";
    }

    private static string MakeRegistryFactoryName(string ns, ClassDeclarationSyntax syntax)
    {
        var prefix = String.IsNullOrEmpty(ns) ? string.Empty : $"{ns}.";
        var suffix = syntax.TypeParameterList is null ? string.Empty : "<>";
        return $"{prefix}{syntax.Identifier}_AccessorFactory{suffix}";
    }
        ...
        List<FactoryEntry> factories)
    {
        ...
        foreach (var factory in factories)
        {
            ...
            //source.Append(factory.AccessorType);
            //source.Append(").GetMethods();");
            //source.AppendLine();
            source.Append("        MemberAccessorGenerator.AccessorRegistry.RegisterFactory(typeof(");
            source.Append(factory.Type);
            source.Append("), typeof(");
            source.Append(factory.AccessorType);
            source.Append("));");
            source.AppendLine();
    ...
    private sealed record FactoryEntry(string Type, string AccessorType);
}
